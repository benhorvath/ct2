...
```{r}
model2 <- lm(data$TARGET_WINS ~ data$TEAM_BATTING_1B + data$TEAM_BATTING_3B + data$TEAM_BATTING_HR + TEAM_BATTING_BB + 
             TEAM_FIELDING_E +TEAM_BASERUN_SB , data=data)
summary(model2)
```
...
Predict data base on model 3 and write to CSV file. 


```{r warning=FALSE}
predicted_targetWins <- predict(model3, newdata=eval_data)
write.csv(predicted_targetWins, file = "Model3-Predcited_TargetWins.csv")
```
