...
#$M_7$: Negative Binomial Full Model 
...
```{r}
summary(m_7 <- glm.nb(TARGET ~ ., data = train_imp))
```
...
#$M_8$: Negative Binomial Regression Model with only `STARS`, `LabelAppeal` and `AcidIndex`. 
...
```{r}
m_8 <- glm.nb(TARGET~ STARS + LabelAppeal + poly(AcidIndex, 3), data = train_imp)
summary(m_8)
```
...
Evaluate each model based on its SOME SCORE on the test set, although we include other metrics as a conveniences:
